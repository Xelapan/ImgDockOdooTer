...
class IrActionsReport(models.Model):
    ...
    def get_paperformat(self):
        logging.warning('self.env.ref ----------------->: %s', self.env.ref('web.minimal_layout', raise_if_not_found=False))
    ...
    def _get_report_url(self, layout=None):
        ...
        logging.warning('report_url ----------------->: %s', report_url)
        logging.warning('layout 3----------------->: %s', layout)
        logging.warning('self 4----------------->: %s', report_url or (layout or self._get_layout() or self).get_base_url())
